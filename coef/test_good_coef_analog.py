...
def coefficients():
    #xlist, ylist = readFile()
    xlist = [-62.01, -26.7, -0.63, 31.88, 72.02, 124.21]
    ylist = [25568, 21179, 19176, 17173, 15070, 12844]
    print(str(len(xlist)) + " __ " + str(len(ylist)))
    xlistNew = xlist
    ylistNew = []
    all_minus = ylist[-1]-200
    for i in ylist:
        ylistNew.append(round((i - all_minus)/2))
    print(xlistNew)
    print(ylistNew)
...
def minimum(xlist, ylist):
    ...
    #x_list_interval_data = xlist
    #y_list_interval_data = ylist
    ...
    new_list_x_test = []
    for i in ylist_test:
        new_list_x_test.append(serch_temp_in_data_list_interpolation(x_list_interval_data, y_list_interval_data, round(float(i) / 16) * 16))
